...
class TournamentParticipant(Base, TimestampMixin):
    ...
    __table_args__ = (
        UniqueConstraint("tournament_id", "player_id", name="uq_tp_tournament_player"),
    )

    id: Mapped[int] = mapped_column(primary_key=True)
    tournament_id: Mapped[int] = mapped_column(
        ForeignKey("tournament.id", name="fk_tp_tournament"), nullable=False
    )
    player_id: Mapped[int] = mapped_column(
        ForeignKey("player.id", name="fk_tp_player"), nullable=False
    )
    ...
    points: Mapped[int] = mapped_column(
        Computed("match_wins * 3 + match_draws", persisted=True), nullable=False
    )
