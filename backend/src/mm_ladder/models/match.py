...
class Match(Base):
    __tablename__ = "match"
    __table_args__ = (CheckConstraint("player_a_id != player_b_id", name="ck_match_different_players"),)

    id: Mapped[int] = mapped_column(primary_key=True)
    tournament_id: Mapped[int] = mapped_column(ForeignKey("tournament.id", name="fk_match_tournament"), nullable=False)
    player_a_id: Mapped[int] = mapped_column(ForeignKey("player.id", name="fk_match_player_a"), nullable=False)
    player_b_id: Mapped[int] = mapped_column(ForeignKey("player.id", name="fk_match_player_b"), nullable=False)
