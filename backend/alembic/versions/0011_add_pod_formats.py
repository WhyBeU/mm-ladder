...
def _recreate_mode() -> str:
    # SQLite can't ALTER constraints in place, so force the copy-and-swap recreation there.
    # On Postgres plain ALTERs work — and recreation fails outright (dropping pk_pod_pod is
    # blocked by the FKs that depend on it).
...
def upgrade() -> None:
    ...
    with op.batch_alter_table("pod_signup", recreate=_recreate_mode()) as batch:
    ...
    with op.batch_alter_table("pod_pod", recreate=_recreate_mode()) as batch:
        batch.add_column(sa.Column("format_id", sa.Integer(), nullable=True))
