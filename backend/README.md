...
Vercel's uv-based builder installs natively — no `requirements.txt` export needed. Dev and
migration tooling stay in Poetry groups (`[tool.poetry.group.*]`), which Vercel ignores. Add
runtime deps with `poetry add <pkg>` as usual (Poetry ≥ 2 writes the `[project]` table).
